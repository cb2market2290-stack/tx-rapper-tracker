...
export async function createToken(userId, label = null) {
  ...
  await query("UPDATE api_tokens SET revoked_at = now() WHERE user_id = ${userId} AND revoked_at IS NULL");
  const raw = generateToken();
  const hash = hashToken(raw);
  const prefix = raw.slice(0, 12);
  const [row] = await query("INSERT INTO api_tokens (user_id, token_hash, prefix, label) VALUES (${userId}, ${hash}, ${prefix}, ${label}) RETURNING id, prefix, created_at");
...
export async function revokeToken(userId, tokenId) {
  const [row] = await query("UPDATE api_tokens SET revoked_at = now() WHERE id = ${tokenId} AND user_id = ${userId} AND revoked_at IS NULL RETURNING id");
  return row ?? null;
}
...
export async function verifyToken(raw) {
  ...
  const hash = hashToken(raw);
  const [row] = await query("SELECT id, user_id FROM api_tokens WHERE token_hash = ${hash} AND revoked_at IS NULL");
  if (!row) return null;
